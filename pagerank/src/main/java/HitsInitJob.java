...
public class HitsInitJob extends Configured implements Tool {
    ...
    public static void main(String[] args) throws Exception {
        int rc = ToolRunner.run(new HitsInitJob(), args);
        System.exit(rc);
    }
...
//        conf.set("urlsFile", urls);
        ...
        job.setMapOutputValueClass(HitsGraphNode.class);

        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(HitsGraphNode.class);

        return job;
    }

    public static class GraphInitMapper extends Mapper<LongWritable, Text, Text, HitsGraphNode> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            for (Element link : links) {
                ...
                if (!urlToDoc.containsKey(normalizedHref)) {
                    ...
                    HitsGraphNode gnNoOutLinks = new HitsGraphNode(i, normalizedHref, true, 1f, 1f);
                    gnNoOutLinks.setOutLinks("[]");
                    context.write(new Text(normalizedHref), gnNoOutLinks);
                }
                i++;
            }
            HitsGraphNode hgn = new HitsGraphNode(documnetId, docUrl, true, 1f, 1f);
            hgn.setOutLinks(outHrefs.toString());
            context.write(new Text(docUrl), hgn);
        }
    }


    public static class GraphInitReducer extends Reducer<Text, HitsGraphNode, Text, HitsGraphNode> {


        @Override
        protected void reduce(Text key, Iterable<HitsGraphNode> values, Context context) throws IOException, InterruptedException {
            HitsGraphNode gf = values.iterator().next();
